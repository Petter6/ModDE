...
class ModularDE:
    ...
    def mutate(self) -> None:
        ...
        if self.parameters.mutation_reference == 'pbest':
            idxs_pbest = np.argsort(self.parameters.population.f)[np.random.randint(np.clip(
                self.parameters.population.n * np.random.uniform(2 / self.parameters.population.n, 0.2,
                                                                 size=self.parameters.population.n), 1,
            ...
            mutated += F * (self.parameters.population[
                                int(np.argmin(self.parameters.population.f))].x - self.parameters.population.x)
        elif self.parameters.mutation_reference == 'rand':
            mutated += F * (self.parameters.population[
                                parent_idxs[:, curr_parent_idx].tolist()].x - self.parameters.population.x)
            curr_parent_idx += 1
        ...
        if self.parameters.use_archive and self.parameters.archive is not None:
            ...
            mutated += F * (self.parameters.population[parent_idxs[:, curr_parent_idx].tolist()].x -
        ...
        for mut_idx in range(n_comps_to_add):
            mutated += F * (self.parameters.population[parent_idxs[:, curr_parent_idx].tolist()].x -
                            self.parameters.population[parent_idxs[:, curr_parent_idx + 1].tolist()].x)
    ...
    def select(self) -> None:
        ...
        """
        x_sel = np.where(self.parameters.population.f < self.parameters.offspring.f, self.parameters.population.x,
                         self.parameters.offspring.x)
        f_sel = np.where(self.parameters.population.f < self.parameters.offspring.f, self.parameters.population.f,
        ...
        self.parameters.improved_individuals_idx = \
            np.where(self.parameters.population.f < self.parameters.old_population.f)[0]
    ...
    def crossover(self) -> None:
        ...
        if self.parameters.crossover == "bin":
            ...
            j_rand = np.random.randint(0, self.parameters.population.x.shape[0],
            ...
            for ind_idx in range(self.parameters.population.n):
                ...
                while offset < self.parameters.population.d:
                    ...
                    crossed[(k + offset) % self.parameters.population.d, ind_idx] = mutated[
    ...
    def fitness_func(self, x: np.ndarray) -> float:
        ...
        if self.parameters.init_stats:
            ...
            if self.parameters.stats.corrected:
                ...
                x_pre = ((self.parameters.crossed[:, idx] - self.parameters.lb) / (
                        self.parameters.ub - self.parameters.lb)).flatten()
                x_target = ((self.parameters.population.x[:, idx] - self.parameters.lb) / (
                        self.parameters.ub - self.parameters.lb)).flatten()
                self.parameters.stats.CS = float(
                    1 - spatial.distance.cosine((x_transformed - x_target), (x_pre - x_target)))
    ...
    def correct_bounds(self) -> np.ndarray:
        ...
        if self.parameters.bound_correction in ['hvb', 'expc_target', 'vector_target']:
            ...
            base_x = (self.parameters.population.x[np.argmin(self.parameters.population.f)] - self.parameters.lb) / (
                    self.parameters.ub - self.parameters.lb)
        else:
            base_x = None
        x_corr = perform_correction((x - self.parameters.lb) / (self.parameters.ub - self.parameters.lb),
                                    self.parameters.out_of_bounds, self.parameters.bound_correction, base_x)
...
def perform_correction(x_transformed, oob_idx, method, base_vector=None):
    ...
    if method == "mirror":
        ...
        x_transformed[oob_idx] = np.abs(2 * (y > 0) - ((y > 0) - np.log(
            1 + np.random.uniform(size=np.sum(oob_idx)) * (np.exp(-1 * np.abs((y > 0) - base_vector[oob_idx])) - 1))))
        ...
        x_transformed[oob_idx] = np.abs(
            2 * (y > 0) - ((y > 0) - np.log(1 + np.random.uniform(size=np.sum(oob_idx)) * (np.exp(-1) - 1))))
